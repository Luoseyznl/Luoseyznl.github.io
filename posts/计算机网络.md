...
## 目录
- [1. 网络体系](#1-网络体系)
  - [1.1 数据链路层设备简介](#11-数据链路层设备简介)
  - [1.2 应用层](#12-应用层)
    - [1.2.1 HTTP 安全和幂等](#121-http-安全和幂等)
    - [1.2.2 HTTP 缓存技术](#122-http-缓存技术)
    - [1.2.3 HTTP 状态码](#123-http-状态码)
    - [1.2.4 HTTP 版本特性](#124-http-版本特性)
    - [1.2.5 HTTPS](#125-https)
    - [1.2.6 RPC](#126-rpc)
- [2. 传输层](#2-传输层)
  - [2.1 TCP 与 UDP](#21-tcp-与-udp)
  - [2.2 TCP 连接管理](#22-tcp-连接管理)
    - [2.2.1 三次握手与四次挥手](#221-三次握手与四次挥手)
    - [2.2.2 SYN Flood 攻击与优化](#222-syn-flood-攻击与优化)
    - [2.2.3 优化 TIME\_WAIT](#223-优化-time_wait)
    - [2.2.4 MSS 分段](#224-mss-分段)
  - [2.3 TCP 重传机制、滑动窗口、流量控制、拥塞控制](#23-tcp-重传机制滑动窗口流量控制拥塞控制)
    - [2.3.1 重传机制](#231-重传机制)
    - [2.3.2 滑动窗口](#232-滑动窗口)
    - [2.3.3 流量控制](#233-流量控制)
    - [2.3.4 拥塞控制](#234-拥塞控制)
...
- **完整性校验**：使用 HMAC（Hash-based Message Authentication Code）对数据进行完整性校验。发送端对数据进行哈希运算生成**数字摘要**，使用私钥加密生成**数字签名**。接收端使用公钥解密后对比摘要，确保数据未被篡改。
- **身份验证**：使用 CA（Certificate Authority）颁发的数字证书验证服务器身份（确保公钥真实可信）。服务器将**数字证书**（公钥、CA 签名）发送给客户端，客户端使用 CA 公钥验证 CA 签名，确保服务器公钥可信。
...
### 2.2.2 SYN Flood 攻击与优化

- **SYN Flood 攻击**  
  攻击者发送大量 SYN 请求，占满服务器的半连接队列（SYN 队列），导致无法处理正常请求。正常情况下，服务器会将接收到 ACK 响应的连接从半连接队列（SYN 队列）移动到已连接队列（Accept 队列）。应用通过调用 accept() 函数从已连接队列中获取连接。
