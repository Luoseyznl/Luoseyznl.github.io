...
- **数据链路层设备简介**
  - **网卡（NIC, Network Interface Card）**
    - 有唯一的 MAC 地址，常见有 Ethernet 网卡、WiFi 网卡等。负责将数据帧发送到物理介质（交换机或路由器）。
  - **交换机（Switch）**
    - 根据 MAC 地址表转发数据帧。连接同一局域网 LAN 内的设备，负责局域网内部通信。局域网指的是同一物理网络内的设备，如办公室、家庭等。
  - **路由器（Router）**
    - 路由器充当了内网和外网之间的网关，外网通常指的是互联网。路由器可以连接多个局域网，并通过 IP 路由协议（如 OSPF、BGP）进行路由选择。
  - **网关（Gateway）**
    - 局域网与广域网之间的连接点，广域网通常指的是互联网。网关可以是路由器、防火墙、代理服务器等。

- **网络请求简要流程**
  1. 域名解析：客户端依次查询本地 DNS 缓存、hosts 文件、本地 DNS 服务器、递归 DNS 服务器，获取目标 IP 地址。
  2. 根据 IP 地址，客户端通过 ARP 协议获取目标设备（目标主机/网关）的 MAC 地址。
  3. 客户端将数据封装为以太网帧，由网卡发送到交换机，交换机根据 MAC 地址转发到目标设备。
